...
class DataFoursquare(object):
    ...
    def filter_users_by_length(self):
        ...
        for u in uid_filtered_sorted:
            ...
            # One entry of pid_n_time: ['4c7c071181bca09382ee0415', '2012-4-16 20:15:54']
            # added by Hussein sort by time in gowalla txt file the check in is not sorted
            #
            pid_n_time = sorted([x for x in pid_n_time], key=lambda x: x[1])
            for i, pid_n_time_enu in enumerate(pid_n_time):
                ...
                try:
                    ...
                    tid = int(time.mktime(time.strptime(tmd, "%Y-%m-%dT%H:%M:%SZ")))
    ...
    def tid_list_48(tmd):
        ...
        tm = time.strptime(tmd, "%Y-%m-%dT%H:%M:%SZ")
